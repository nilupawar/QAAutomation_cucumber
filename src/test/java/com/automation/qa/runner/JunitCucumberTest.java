...
@CucumberOptions(
        ...
        plugin = {"pretty"                                  // generates console output with details of gherkin steps. if skipped you will not be able to explicitly see executed gherkin steps on console output
                ,"html:target/cucumber_html_output"         // generates execution reports in html format
                ,"json:target/cucumber_json_report.json"    // generates execution reports in json format. Usually this report can be used by other plugins to generate useful reports
                ,"progress:target/cucumber_progress_report" //
                ,"rerun:target/cucumber_rerun_report"}, // generates a file which has details about all failed test scenario. This generated rerun file can be used by another test runner to rerun only the failed scenarios
        tags = {"@Scenario1 or @Scenario2"}, // Selects test scenario for execution based on the specified tags. to create an OR condition use {"@ORtag1,@ORtag2"} OR {"@ORtag1 or @ORtag2"} and to create a OR condition use {"@ANDtag1","@ANDtag2"}. use {"~@NotToExecuteTag"}
